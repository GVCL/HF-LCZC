...
from tensorflow.keras.layers import MaxPool2D, GlobalAveragePooling2D, BatchNormalization, Conv2D,UpSampling2D 
...
# CNN model for sentinel-1 modality
...
# CNN model for sentinel-2 modality
...
#Pixel level fusion; Concatenate the raw images along the channel axis
concatenated_raw_data = Concatenate()([input_layer_s1, input_layer_s2])

# Convolutional layers, pooling, ReLU activation and Normalization
pix = Conv2D(32, kernel_size, strides=stride_size, padding='same', activation='relu')(concatenated_raw_data)
pix = MaxPool2D(pool_size=pool_size, strides=(2, 2))(pix)
pix = BatchNormalization()(pix)

# More convolutional layers and pooling
pix = Conv2D(64, kernel_size, strides=stride_size, padding='same', activation='relu')(pix)
pix = MaxPool2D(pool_size=pool_size, strides=(2, 2))(pix)
pix = BatchNormalization()(pix)

pix = Conv2D(128, kernel_size, strides=stride_size, padding='same', activation='relu')(pix)
pix = MaxPool2D(pool_size=pool_size, strides=(2, 2))(pix)
pix = BatchNormalization()(pix)

# Feature-level fusion; Concatenate features from both modalities
concatenated_features = Concatenate()([x_s1, x_s2]) 

# More convolutional layers, pooling, ReLU activation and Normalization
...
x = Conv2D(18, kernel_size, strides=stride_size, padding='same', activation='relu')(x)
x = MaxPool2D(pool_size=pool_size, strides=(2, 2))(x)
x = BatchNormalization()(x)

x = UpSampling2D(size=(4, 4), data_format=None, interpolation='nearest')(x)

# Hybrid fusion; Combine both data-level and feature-level fusion
...
model.save("Hyb-100.h5")

model = tf.keras.models.load_model("Hyb-100.h5")
try:
	data = []
